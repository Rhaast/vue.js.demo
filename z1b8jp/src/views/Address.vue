...
<script>
  ...
  export default{
   ...
   mounted(){
          this.init();
        },      //mounted中初始化this.init()方法，就去渲染了user/addressList这个接口
    computed:{   //computed计算属性函数
      addressListFilter(){    //设置地址默认显示条数，条数保存在data的limit里面
      ...
      setDefault(addressId){    //设置默认地址
        ...
        }).then((response)=>{   //设置默认地址
          let res = response.data;
          if(res.status=='0'){
            console.log("set default");
            this.init(); //调用init可以使函数在运行完时自动重载运行后的结果
          }
      ...
      delAddressComfirm(addressId){         //弹出模态框
        this.isMdShow = true;
